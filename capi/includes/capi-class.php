...
//error_log("capi-class construct wiwid:".$this->wiwid);
  }  // end construct()
...
//error_log("process api operation: $operation queryin: $queryin");
...
//	error_log("user_cid:$user_cid");
		  ...
		  if ($queryin>"") $query=$this->urlencodeall($queryin)."&"; else $query="";
...
//error_log("session cid:".$_SESSION["cid"]);
			...
			$my_return=array("message"=>$message,
	               "success"=>$success,
	               "uid"=>$user_id,
	               "cid"=>$_SESSION["cid"],
		 						);
...
//		$output.="<textarea>$result</textarea>";
	...
	function capi_do_check_in() {
		...
		if ($button_text!="Check In" && $button_text!="Check Out") {
			...
			if ($button_text=="Check In") $checked_in=0; else $checked_in=1;
		...
		//      $output.= capi_watc_theme_input($form,"","pmcpid","hidden",$my_pm_contact_process_bll->get_field("pmcpid"));
		      ...
		      $output.= $this->capi_theme_input($form,array("field"=>"operation","input_type"=>"hidden","type"=>"s","default"=>"save_site_contact_subscription"));
					...
					foreach ($subscriptions as $subscription) {
						...
						if (isset($sid[$subscription['sid']])) $default=1; else $default=0;
	...
	//print_r($_REQUEST);
		  ...
		  $capi_pw = $this->capi_pw;
			...
			if (!($un==$capi_un && $pw==$capi_pw)) {
				$my_return=array("message"=>"validation error",
		               "success"=>false,
			 						);
			...
			if ($my_user=get_user_by( 'login', $wiu_user_name )) {
				...
				if ($is_user) {
					$my_return=array("message"=>"User exists: $wiu_user_name",
			               "success"=>false,
				 						);
			...
			if ($my_user=get_user_by( 'email', $email )) {
				...
				if ($is_user) {
					$my_return=array("message"=>"User email exists error: $email",
			               "success"=>false,
				 						);
	...
	//print_r($_REQUEST);
		  ...
		  $capi_pw = $this->capi_pw;
			...
			if (!$my_user=get_user_by( 'id', $uid )) {
				$my_return=array("message"=>"User does not exist: $uid",
		               "success"=>false,
			 						);
...
//print_r($_REQUEST);
		  ...
		  $capi_pw = $this->capi_pw;
			...
			if ($group_names=="") {
				$my_return=array("message"=>"Error. No groups specified.",
		               "success"=>false,
			 						);
...
error_log("deletewebsiteuser:".print_r($queryin,true));
	//print_r($_REQUEST);
		  ...
		  $capi_pw = $this->capi_pw;
			...
			if ($external_uid==1) {
				$my_return=array("message"=>"Can't delete user 1",
		               "success"=>false,
			 						);
			...
			if (!$my_user=get_user_by( 'id', $external_uid )) {
				$my_return=array("message"=>"User does not exist: $external_uid",
		               "success"=>false,
			 						);
...
//	error_log("field: $field  default: ".print_r($default,true));
	  ...
	  $output="";
	  if ($maxlength>45) $size=22.5; else if ($type=="date") $size=$maxlength; else $size=$maxlength/2;
	  if (isset($options["rows"])) $rows=$options["rows"]; else $rows=10;
	  if (isset($options["cols"])) $cols=$options["cols"]; else $cols=60;
...
//							location.reload();
	...
	function capi_mass_subscribe() {
		...
		//      $output.= capi_watc_theme_input($form,"","pmcpid","hidden",$my_pm_contact_process_bll->get_field("pmcpid"));
		      ...
		      $output.= $this->capi_theme_input($form,array("label"=>"Membership Status:","field"=>"status_flag","input_type"=>"select","type"=>"s","default"=>"A",
							"options"=>array("A"=>"Active","I"=>"Inactive","B"=>"Both")));

		      $output.= $this->capi_theme_input($form,array("label"=>"Golfer Status:","field"=>"member_type_flag","input_type"=>"select","type"=>"s","default"=>"R",
							"options"=>array("R"=>"Regular","J"=>"Junior","B"=>"Both")));
				
		      $output.= $this->capi_theme_input($form,array("label"=>"Membership Type:","field"=>"member_type_flag","input_type"=>"select","type"=>"s","default"=>"R",
	      ...
	      $output.= $this->capi_theme_input($form,array("label"=>"Subscribe To:","field"=>"sid","input_type"=>"select","type"=>"s","default"=>"",
						"options"=>$subscription_array));

	      $output.= $this->capi_theme_input($form,array("label"=>"Empty List Before Subscribing:","field"=>"clear_first","input_type"=>"select","type"=>"s","default"=>"N",
